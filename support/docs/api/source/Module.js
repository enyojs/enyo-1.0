...
enyo.kind({
	...
	//* Show only the official documentation when `true`. When `false`, show only the unofficial / undocumented parts of enyo.
	showOfficial: true,
	...
	create: function() {
		...
		this.showOfficial = !Boolean(enyo.args.unofficial);
	...
	indexObjects: function() {
		...
		for (var i=0, c, name; c=objects[i]; i++) {
			if ((this.showOfficial ? c.group == "public" : true) && c.name) {
				switch (c.type) {
					case "kind":
						name = c.name.value;
						break;
	...
	formatModule: function(inPath, inModule) {
		...
		for (var i=0, c; c=objects[i]; i++) {
			if (c.group !== "public" && this.showOfficial) {
				continue;
			}
			switch (c.type) {
				case "function":
	...
	addSimplePropertyList: function(inProperties, inTitle, inHtml) {
		...
		if (inProperties && inProperties.names) {
			...
			for (var i=0, n; n=inProperties.names[i]; i++) {
				if (this.showOfficial ? inProperties.map[n].group == "public" : true) {
					public.push(n);
				}
			}
			if (public.length) {
	...
	allowVisible: function(inDocFragment) {
		if (this.showOfficial) {
			return inDocFragment.group == "public";
		}
		return inDocFragment.group != "public" || !inDocFragment.comment;
	}
})
...
enyo.Module.buildModuleList = function() {
	var map = {};
...
enyo.Module.buildToc = function() {
	var map = enyo.Module.buildModuleList();
