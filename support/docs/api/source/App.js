enyo.kind({
	...
	selectTopic: function(inTopic) {
		this.topic = inTopic;
		var c = enyo.Module.topicMap[inTopic];
		var a;
		if (c) {
			this.$.radioGroup.setValue(1);
			this.$.pane.selectView(this.$.index, true);
			a = enyo.byId("idx_" + inTopic);
			if (a) {
				a.scrollIntoViewIfNeeded();
			}
		}
		else {
			c = this.modules[inTopic];
			this.$.radioGroup.setValue(0);
			this.$.pane.selectView(this.$.toc, true);
			a = enyo.byId("toc_" + inTopic);
			if (a) {
				a.scrollIntoViewIfNeeded();
			}
		}
