...
enyo.kind({
	...
	published: {
		...
		*/
		...
		If a request times out, the request object will have its didTimeout property set to true. A zero
		value indicates that no timeout will be checked.
		*/
		timeout: 0
	},
	events: {
		...
		onFailure: "",
		//* (inSender, inResponse, inRequest) event sent always when request returns, success or not
		onResponse: ""
	},
	requestKind: "Request",
	//* @protected
...
enyo.kind({
	...
	/** called to determine if the response was a failure, 
		usually overridden by specific Request types */
