...
enyo.kind({
	...
	// prevents automatic click squelching when the user drags (links are prevented here by handling clicks, therefore
	// this control needs *all* clicks and must perform any needed squelching itself)
	requiresDomClick: true,
	allowHtml: true,
	//* @protected
	create: function() {
	...
	clickHandler: function(inSender, inEvent) {
		...
		// due to requiresDomClick, we will always get a click event.
		// if there's a link, stop the click.
		if (url) {
			this.doLinkClick(url, inEvent);
			inEvent.preventDefault();
			return true;
		// stop the click if there was a drag.
		} else if (inEvent.didDrag) {
			return true;
		// otherwise allow the click.
		} else {
			return this.doClick();
		}
	}
