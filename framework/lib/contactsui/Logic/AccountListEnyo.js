...
enyo.kind({
	name: "com.palm.library.contactsui.AccountList",
	kind: "Component",
	published: {},
	events: {
	},
	...
	ready: function () {
		enyo.log("------------in AccountList ready method-------------");     
		this.refresh();
	},
	...
	refresh : function () {
		...
		this._listObj = this.$.getAccounts.getAccounts({
			// Note: We can't filter by 'CONTACTS' only since IM accounts are needed, and if we filter with both 'CONTACTS' & 'MESSAGING' we get the wrong icons
			//		 This is less efficeint but there is no good alternative at the moment, we can come back and see if there is a better solution later...
			//capability: "CONTACTS"
			//capability: ['CONTACTS', 'MESSAGING']
		});
	},

	constructor : function (onReady) {

		enyo.log("------------in AccountList constructor method-------------");     
		this.inherited(arguments);
		this.list = [];
...
//		Contacts.Utils.mixInBroadcaster(this);
	...
	removeDefaultAccountsListener : function (callback) {
		...
		if (i !== -1) {
			...
			enyo.error("removeDefaultAccountsListener: Cannot find callback to remove.");
		}
	},
	...
	getAccountIcon : function (accountId, kindName) {
		...
		if (icon && kindName && kindName === "com.palm.contact.libpurple:1") {
			if (icon.lastIndexOf("google-") > 0) {
				icon = icon.replace("google-", "gtalk-");
			}
			else if (icon.lastIndexOf("aol-") > 0)
			{
				icon = icon.replace("aol-", "aim-");
			}
			else if (icon.lastIndexOf("yahoo-") > 0)
			{
				icon = icon.replace("yahoo-", "yim-");
			}
		}
	...
	*/
	_accountsChanged : function (inSender, inResponse) {
		...
		this.list = accounts;
		enyo.log("ContactsApp._accountsChanged: accounts #:" + accounts.length);
		...
		accounts.forEach(function (account) {
			...
			account.capabilityProviders.forEach(function (capabilityProvider) {
				...
				// !!capabilityProvider._id determines if the capabilityProvider is enabled
				if (capabilityProvider && capabilityProvider.capability === "CONTACTS" && !!capabilityProvider._id && !capabilityProvider.readOnlyData && account.templateId !== "com.palm.sim") {
					//enyo.log(templateId + "is NOT read only!!");
...
com.palm.library.contactsui.AccountList.defaultAccountPriority = ["com.palm.eas", "com.palm.google"];
