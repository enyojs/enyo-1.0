...
enyo.kind({
	...
	components: [
			...
			{kind: "Scroller", flex:1, components: [
				{kind:"Control", name: "noAccountsSetUpYet", style:"width:100%", components: [
					{kind:"Control", className:"box-center", components:[
						{name: "getStartedWithProfileAccount", components: [
							...
							{kind: "Button", className: "enyo-button-dark accounts-btn", name: "profileButton", onclick: "doAccountsFirstLaunchDone"},
						...
						{name: "getStartedWithLocalStorageAccount", components: [
							...
							{kind: "Button", className: "enyo-button-dark accounts-btn", content: AccountsUtil.BUTTON_GO, onclick: "doAccountsFirstLaunchDone"},
							{content: AccountsUtil.TEXT_OR_ADD_NEW_ACCOUNT, className:"accounts-body-title"}
						]},
				...
				{kind:"Control", name: "oneOrMoreAccounts", style:"width:100%", components: [
					{kind:"Control", className:"box-center", components:[
						...
						{kind: "enyo.Button", className: "enyo-button-dark accounts-btn", label:AccountsUtil.BUTTON_GO, onclick: "doAccountsFirstLaunchDone"},
						{content: AccountsUtil.TEXT_OR_ADD_NEW_ACCOUNT, className:"accounts-body-title"},
		...
		{kind: "AccountsModify", name: "AccountsModify", onAccountsModify_Done: "modifyDone"},
		{kind: "Accounts.crossAppUI", name:"customAccountsUI", onResult: "handleCustomUIResult", tabIndex: -1},
	...
	AddAccount: function(button) {
		var cap = (enyo.isArray(this.capability)? this.capability[0]: this.capability);
	...
	editAccount: function(inSender, inResults) {
		// If the credentials are in error then allow the user to enter credentials
		if (inResults.account.credentialError) {
			// Allow the user to enter account credentials, or delete the account
			this.selectViewByName("AccountsModify");
			this.$.AccountsModify.ModifyAccount(inResults.account, null, this.capability);
		}
	},
